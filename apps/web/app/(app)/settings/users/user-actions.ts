...
export async function updateUserRoleAction(input: unknown): Promise<ActionResult<null>> {
  try {
...
export async function setUserStatusAction(input: unknown): Promise<ActionResult<null>> {
  try {
    const me = await requireRole(['admin']);
    ...
    if (status === 'suspended' && target.role === 'admin' && target.status === 'active') {
      const remaining = await activeAdminCount(me.organization_id);
      if (remaining <= 1) {
        throw new ActionError('LAST_ADMIN', 'You cannot suspend the last admin.');
      }
    }
