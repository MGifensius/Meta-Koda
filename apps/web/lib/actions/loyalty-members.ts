...
export async function enrollMemberAction(customerId: string): Promise<ActionResult<null>> {
  try {
    ...
    if (!data || data.length === 0) {
      throw new ActionError('NOT_UPDATED', 'Customer not found or RLS-blocked.');
    }
...
export async function unenrollMemberAction(customerId: string): Promise<ActionResult<null>> {
  try {
